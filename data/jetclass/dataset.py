...
def _finalize_inputs(table, data_config):
    ...
    for k, params in data_config.preprocess_params.items():
        ...
        if np.any(np.isnan(table[k])):
            print(f"Found NaN in {k}, silently converting it to 0.")
...
class _SimpleIter(object):
    ...
    def restart(self):
        ...
        #print(f"Init iter [{0 if self.worker_info is None else self.worker_info.id}],\
        #        will load {len(self.filelist)} (out of {len(sum(self._init_file_dict.values(), []))}*{self._file_fraction}={int(len(sum(self._init_file_dict.values(), [])) * self._file_fraction)})\
        #        files with load_range={str(self.load_range)}:\n{', '.join(self.filelist[: 3]) + ' ... ' + self.filelist[-1]}")
        #print(f"Restarted DataIter {self._name}, load_range={str(self.load_range)}, file_list:\n{json.dumps(self.worker_file_dict, indent=2)}")
