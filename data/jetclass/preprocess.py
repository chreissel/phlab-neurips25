...
def _build_weights(table, data_config, reweight_hists=None):
    ...
    if data_config.use_precomputed_weights:
        ...
        if sum_evts != len(table):
            print('Not all selected events used in the reweighting. ')
            print('Check consistency between `selection` and `reweight_classes` definition, or with the `reweight_vars` binnings ')
            print('(under- and overflow bins are discarded by default, unless `reweight_discard_under_overflow` is set to `False` in the `weights` section).')
...
class AutoStandardizer(object):
    ...
    def read_file(self, filelist):
        ...
        print('[AutoStandardizer] keep_branches:\n  %s' % ','.join(keep_branches))
        print('[AutoStandardizer] aux_branches:\n  %s' % ','.join(aux_branches))
        print('[AutoStandardizer] load_branches:\n  %s' % ','.join(load_branches))
    ...
    def make_preprocess_params(self, table):
        print('Using %d events to calculate standardization info' % len(table))
            ...
            if params['center'] == 'auto':
                if k.endswith('_mask'):
                    ...
                    if np.any(np.isnan(a)):
                        print('[AutoStandardizer] Found NaN in `%s`, will convert it to 0.' % k)
                        time.sleep(10)
                        a = np.nan_to_num(a)
                ...
                print('[AutoStandardizer] %s low=%s, center=%s, high=%s, scale=%s' % (k, low, center, high, scale))
...
class WeightMaker(object):
    ...
    def read_file(self, filelist):
        ...
        print('[WeightMaker] keep_branches:\n  %s' % ','.join(keep_branches))
        print('[WeightMaker] aux_branches:\n  %s' % ','.join(aux_branches))
        print('[WeightMaker] load_branches:\n  %s' % ','.join(load_branches))
    ...
    def make_weights(self, table):
        ...
        print('Using %d events to make weights' % len(table))
        ...
        if sum_evts != len(table):
            print(
                'Only %d (out of %d) events actually used in the reweighting. ' %  (sum_evts, len(table)),
        ...
        if self._data_config.reweight_method == 'flat':
            for label, classwgt in zip(self._data_config.reweight_classes, self._data_config.class_weights):
                ...
                print('label:%s, median=%f, min=%f, ref=%f, ref/min=%f' % (label, med_val, min_val, ref_val, ref_val / min_val))
        ...
        if self._data_config.reweight_basewgt:
            ...
            print('Sample weight percentiles: %s' % str(np.percentile(wgts, np.arange(101))))
            wgt_ref = np.percentile(wgts, 100 - self._data_config.reweight_threshold)
            print('Set overall reweighting scale factor (%d threshold) to %s (max %s)' % (100 - self._data_config.reweight_threshold, wgt_ref, np.max(wgts)))
            for label in self._data_config.reweight_classes:
    ...
    def produce(self, output=None):
        ...
        if output:
            print('Writing YAML file w/ reweighting info to %s' % output)
            self._data_config.dump(output)
        return self._data_config
