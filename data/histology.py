...
### dataloading copied from here: https://github.com/nswood/anomaly-detection-in-histology/blob/master/utils/image_dataset_reader.py
...
class color_transform_skimage():
    ...
    def __init__(self, int_type_precision=np.uint8):
        ...
        self.tr_type = float  # type of output of color skimage transformation
    ...
    def _digitize(self, im):
        ...
        for i, im_channel in enumerate(im):
            ...
            factor = float(self.max_intensity) / (self.tr_max[i] - self.tr_min[i])
    ...
    def _undigitize(self, im):
        ...
        for i, im_channel in enumerate(im):
            ...
            factor = (self.tr_max[i] - self.tr_min[i]) / float(self.max_intensity)
...
class HistImagesDataset(Dataset):
    ...
    def random_split(self, frac, transform_validation='same', val_dataset_name='', train_dataset_name='', shuffle=True):
